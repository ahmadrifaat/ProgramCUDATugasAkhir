...
// Variabel global untuk kontrol pause
bool paused = false;
...
__global__ void update_eta_kernel_stable(float* eta_new, const float* eta, const float* p, const float* q, const float* z, const int* valid_cell, const float* rain_source, float dt) {
    ...
    if (isfinite(temp_eta_new)) {
        if (temp_eta_new < z[idx]) {
            eta_new[idx] = z[idx];
        } else {
            eta_new[idx] = temp_eta_new;
        }
    } else {
        eta_new[idx] = eta[idx];
    }
...
int main(int argc, char *argv[]) {
    ...
    GLFWwindow *window = glfwCreateWindow(window_width, window_height, "Simulasi Banjir CUDA", NULL, NULL);
    ...
    printf("Mengalokasikan memori di GPU...\n");
    ...
    FILE *logfile = fopen("log_simulasi_final.csv", "w");
