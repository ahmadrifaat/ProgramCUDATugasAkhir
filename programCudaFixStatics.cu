...
#define STATIC_RETURN_PERIOD_YEARS 50.0f
...
// Fungsi-fungsi terkait hujan yang telah diformat ulang
...
int main(int argc, char *argv[]) {
    ...
    printf("Versi: Interaktif dengan Pause/Resume\n");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!paused && !simulation_finished) {
            ...
            if (current_sim_time >= TOTAL_SIMULATION_DURATION_SECONDS) {
                ...
                CUDA_CHECK(cudaMemcpy(d_rain_source, rain_source, size_float, cudaMemcpyHostToDevice));
    ...
    printf("Waktu Eksekusi (Wall Clock) : %.4f detik\n", wall_clock_time_used);
    printf("Total Langkah Simulasi      : %d langkah\n", total_steps_taken);
    if (total_steps_taken > 0) {
        printf("Waktu Rata-rata/Langkah     : %.6f detik/langkah\n", wall_clock_time_used / total_steps_taken);
    }
    if (wall_clock_time_used > 0) {
        printf("FPS Rata-rata               : %.2f frame/detik\n", total_steps_taken / wall_clock_time_used);
    }
    printf("--------------------------------------------------------\n");
