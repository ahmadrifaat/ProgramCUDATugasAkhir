...
#define NX 3162
#define NY 3162
#define DX 3.952561345792f
#define DY 4.680042736347f
#define DT 0.01f 
#define G 9.81f
...
#define MIN_DT 1e-8f
#define TOTAL_SIMULATION_DURATION_SECONDS 10000.0f 
#define SLOPE_TOLERANCE 1e-7f
#define REDUCTION_BLOCK_SIZE 256
#define LOG_INTERVAL 1000


#define PRD_INITIAL_INTENSITY_MM_HR 50.0f 
...
// Pointer Device (GPU)
float *d_z, *d_eta, *d_eta_new, *d_p, *d_q, *d_n_manning;
int *d_valid_cell;
float *d_rain_source;
float *d_overlay;
...
const char *fragment_shader_src = "#version 330 core\n"
    ...
    "           float norm_eta = (eta_val - u_min_z) / (u_max_z - u_min_z + u_max_vis_water_depth);"
    "           norm_eta = clamp(norm_eta, 0.0, 1.0);"
    "           base_color = vec4(mix(vec3(0.1, 0.6, 0.2), vec3(0.0, 0.0, 0.55), norm_eta), 1.0);"
    "       } else if (h < u_threshold) {" 
    "           if (u_max_z > u_min_z) {"
...
void init_gl(int width, int height) {
    ...
    // --- PERBAIKAN ---
    loc_visualization_mode = glGetUniformLocation(shader_program, "u_visualization_mode");
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER, NX * NY * sizeof(float), NULL, GL_DYNAMIC_DRAW);
...
__global__ void copy_to_pbo_kernel(float* pbo_ptr, const float* eta) {
...
void render_gl() {
    ...
    CUDA_CHECK(cudaGraphicsResourceGetMappedPointer((void**)&pbo_ptr, &num_bytes, cuda_pbo_resource));

    dim3 blockSize(16, 16);
    dim3 gridSize((NX + blockSize.x - 1) / blockSize.x, (NY + blockSize.y - 1) / blockSize.y);
    copy_to_pbo_kernel<<<gridSize, blockSize>>>(pbo_ptr, d_eta);
    ...
    glBindTexture(GL_TEXTURE_2D, tex_eta);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, pbo);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, NX, NY, GL_RED, GL_FLOAT, 0);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
    ...
    // --- PERBAIKAN ---
    glUniform1i(loc_visualization_mode, visualization_mode);
...
int read_dem(const char *filename) {
    ...
    // --- PERBAIKAN: Memvalidasi konstanta yang di-hardcode ---
    if (ncols != NX || nrows != NY) {
        fprintf(stderr, "Error: Dimensi DEM (%dx%d) tidak cocok dengan NXxNY (%dx%d).\n", ncols, nrows, NX, NY);
        fprintf(stderr, "       Silakan kompilasi ulang program dengan nilai NX/NY yang benar di header!\n");
        fclose(file);
        return 0;
    }
    if (fabsf(dem_dx - DX) > 1e-4f || fabsf(dem_dy - DY) > 1e-4f) {
         fprintf(stderr, "Peringatan: dx/dy DEM (%.6f, %.6f) tidak cocok dengan DX/DY di header (%.6f, %.6f).\n", dem_dx, dem_dy, DX, DY);
         fprintf(stderr, "         Menggunakan nilai DX/DY dari header.\n");
    }
...
void save_eta_as_asc(const char* filename) {
    CUDA_CHECK(cudaMemcpy(eta, d_eta, NX * NY * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    // --- PERBAIKAN: Menggunakan konstanta yang benar untuk header ---
...
float get_scs_type2_simplified_intensity_mm_per_hour(float time_in_hours, float total_rainfall_mm, float storm_duration_hours, float peak_time_ratio) {
    ...
    if (time_in_hours >= 0 && time_in_hours <= storm_duration_hours) {
        if (time_in_hours <= peak_time_hours) {
            if (peak_time_hours > 1e-6) { // --- PERBAIKAN: Menghindari pembagian dengan nol ---
...
//kernel untuk random generator CUDA
...
// tambahan revisi hitung hujan di GPU
__global__ void compute_rainfall_kernel(float *rain_source, const int *valid_cell, int pattern, float time, float uniform_intensity, curandState *rand_states) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (!valid_cell[idx]) {
        rain_source[idx] = 0.0f;
        return;
    }

    switch (pattern) {
        case 0: 
            rain_source[idx] = (0.05f * curand_uniform(&rand_states[idx])) / 3.6e6f;
            break;
        case 1: { 
            ...
            if (distance < radius) {
                rain_source[idx] = intensity * (1.0f - distance / radius);
            } else {
                rain_source[idx] = 0.0f;
            }
            break;
        ...
        case 4: // SCS 
        case 5: // ABM 
            rain_source[idx] = uniform_intensity;
            break;
        default:
            rain_source[idx] = 0.0f;
            break;
...
__device__ float sign_device(float x) {
    return (x > 0.0f) - (x < 0.0f);
}

__global__ void compute_flux_kernel_enhanced_stability(float* p, float* q, const float* eta, const float* z, const int* valid_cell, const float* n_manning) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;

    if (i >= NX - 1 || j >= NY - 1 || i == 0 || j == 0) return;
    ...
    int idx_e = (i + 1) + j * NX;
    if (valid_cell[idx_e]) {
        float d_eta = eta[idx_e] - eta[idx];

        if (fabsf(d_eta) > SLOPE_TOLERANCE) {
            float z_face = fmaxf(z[idx], z[idx_e]);
            float eta_up = fmaxf(eta[idx], eta[idx_e]);
            float h_face = eta_up - z_face;

            if (h_face > THRESHOLD) {
                float flux_adv = 0.0f;
                float n_avg = 0.5f * (n_manning[idx] + n_manning[idx_e]);
                if (n_avg > 1e-6f) {
                    float S = d_eta / DX;
                    ...
                    flux_adv = -v_final * h_face * sign_device(S);
                }

                float h_L = fmaxf(0.0f, eta[idx] - z[idx]);
                float h_R = fmaxf(0.0f, eta[idx_e] - z[idx_e]);
                float avg_h_for_diff = 0.5f * (h_L + h_R);
                float D = DIFFUSION_COEFF * DX * sqrtf(G * avg_h_for_diff);
                float flux_diff = -D * (h_R - h_L) / DX;

                p[idx] = flux_adv + flux_diff;
            }
        }
    }
    ...
    int idx_n = i + (j + 1) * NX;
    if (valid_cell[idx_n]) {
        float d_eta = eta[idx_n] - eta[idx];

        if (fabsf(d_eta) > SLOPE_TOLERANCE) {
            float z_face = fmaxf(z[idx], z[idx_n]);
            float eta_up = fmaxf(eta[idx], eta[idx_n]);
            float h_face = eta_up - z_face;

            if (h_face > THRESHOLD) {
                float flux_adv = 0.0f;
                float n_avg = 0.5f * (n_manning[idx] + n_manning[idx_n]);
                if (n_avg > 1e-6f) {
                    float S = d_eta / DY;
                    float v_manning = (1.0f / n_avg) * powf(h_face, 0.6666667f) * sqrtf(fabsf(S));
                    float v_crit = sqrtf(G * h_face);
                ...
                float h_S = fmaxf(0.0f, eta[idx] - z[idx]);
                float h_N = fmaxf(0.0f, eta[idx_n] - z[idx_n]);
                float avg_h_for_diff = 0.5f * (h_S + h_N);
                float D = DIFFUSION_COEFF * DY * sqrtf(G * avg_h_for_diff);
                float flux_diff = -D * (h_N - h_S) / DY;

                q[idx] = flux_adv + flux_diff;
            }
        }
    }
}

__global__ void update_eta_kernel_stable(float* eta_new, const float* eta, const float* p, const float* q, const float* z, const int* valid_cell, const float* rain_source, float dt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (!valid_cell[idx] || i == 0 || j == 0 || i >= NX - 1 || j >= NY - 1) {
        eta_new[idx] = eta[idx];
        return;
    }

    // Ambil fluks "keinginan" dari kernel compute_flux
    float p_in = p[idx - 1]; 
    float p_out = p[idx];
    float q_in = q[idx - NX];
    float q_out = q[idx];

    float rain_term = rain_source[idx];
    
    // Hitung prediksi "naif" (seperti di V1)
    float dpdx = (p_out - p_in) / DX;
    float dqdy = (q_out - q_in) / DY;
    float temp_eta_new = eta[idx] - dt * (dpdx + dqdy) + dt * rain_term;
    
    if (isfinite(temp_eta_new)) {
        
        // --- INI ADALAH SATU-SATUNYA LOGIKA YANG KITA GUNAKAN ---
        
        // 1. Hitung total volume KELUAR dan MASUK (dalam meter kedalaman)
        //    (Deklarasi PERTAMA dan SATU-SATUNYA)
        float p_out_total = fmaxf(0.0f, p_out) + fmaxf(0.0f, -p_in); // Total aliran keluar X
        float q_out_total = fmaxf(0.0f, q_out) + fmaxf(0.0f, -q_in); // Total aliran keluar Y
        
        float p_in_total = fmaxf(0.0f, -p_out) + fmaxf(0.0f, p_in); // Total aliran masuk X
        float q_in_total = fmaxf(0.0f, -q_out) + fmaxf(0.0f, q_in); // Total aliran masuk Y

        float total_outflow_vol_m = dt * ( (p_out_total / DX) + (q_out_total / DY) );
        float total_inflow_vol_m  = dt * ( (p_in_total / DX) + (q_in_total / DY) ) + (dt * rain_term);
        
        // (Deklarasi PERTAMA dan SATU-SATUNYA)
        float h_available = fmaxf(0.0f, eta[idx] - z[idx]);

        float scaling_factor = 1.0f;
        
        // PERBAIKAN Logika: Kita hanya bisa mengirim 'h_available', 
        // inflow akan ditambahkan nanti.
        if (total_outflow_vol_m > h_available) {
             // Overdraft AKAN terjadi
            if (total_outflow_vol_m > 1e-9f) {
                // Kecilkan aliran keluar agar HANYA menghabiskan h_available
                scaling_factor = h_available / total_outflow_vol_m;
            } else {
                scaling_factor = 0.0f;
            }
        }
        
        // 2. Hitung perubahan eta (dh) dengan fluks keluar yang sudah di-skala
        float dh = total_inflow_vol_m - (total_outflow_vol_m * scaling_factor);
        
        float temp_eta_new_scaled = eta[idx] + dh;

        // 3. Set eta baru, dengan jaring pengaman fmaxf
        // (Jaring pengaman ini sekarang seharusnya jarang sekali aktif)
        eta_new[idx] = fmaxf(z[idx], temp_eta_new_scaled);
...
void simulate_step_cuda(float dt) {
    size_t size_float = NX * NY * sizeof(float);
    CUDA_CHECK(cudaMemset(d_p, 0, size_float));
    CUDA_CHECK(cudaMemset(d_q, 0, size_float));

    dim3 blockSize(16, 16);
    dim3 gridSize((NX + blockSize.x - 1) / blockSize.x, (NY + blockSize.y - 1) / blockSize.y);

    // --- PERBAIKAN: Berikan 'dt' ke kernel fluks ---
    compute_flux_kernel_enhanced_stability<<<gridSize, blockSize>>>(d_p, d_q, d_eta, d_z, d_valid_cell, d_n_manning);
    CUDA_CHECK(cudaGetLastError());

    update_eta_kernel_stable<<<gridSize, blockSize>>>(d_eta_new, d_eta, d_p, d_q, d_z, d_valid_cell, d_rain_source, dt);
    CUDA_CHECK(cudaGetLastError());

    CUDA_CHECK(cudaMemcpy(d_eta, d_eta_new, size_float, cudaMemcpyDeviceToDevice));
}
...
void write_log(FILE *logfile, int step, float time, float intensity_mm_hr, float cumulative_rainfall_mm, float *out_max_h, float *out_min_h) {
    ...
    for (int j = 0; j < NY; j++) {
        for (int i = 0; i < NX; i++) {
            ...
            // --- PERBAIKAN: Hanya hitung volume jika h > 0 (mengabaikan noise kecil) ---
            if (h > 0.0f) { 
                eta_volume += h * DX * DY; // Gunakan konstanta DX/DY yang benar
            }
            if (h > max_h) max_h = h;
            if (h > 0.0f && h < min_h) min_h = h; 
        }
    }
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS) {
        ...
        printf("Mode visualisasi diubah ke: %s\n", (visualization_mode == 0) ? "KEDALAMAN (h)" : "KETINGGIAN (eta)");
    }
    }
}
int main(int argc, char *argv[]) {
    ...
    printf("Program dikompilasi untuk CUDA dengan Interoperabilitas OpenGL.\n");
    printf("Versi: Interaktif dengan Kalkulasi Hujan di GPU dan Profiling Performa\n");
    printf("--- VERSI PERBAIKAN: Mass-Conservative Flux Limiter ---\n");
    ...
    printf("Membaca DEM: %s\n", argv[1]);
    if (!read_dem(argv[1])) { return 1; }
    printf("Membaca Manning: %s\n", argv[2]);
    if (!read_manning_map(argv[2])) { return 1; }

    // --- TAMBAHKAN KODE INI ---
    printf("\nMenghitung luas area DAS yang valid...\n");
    
    // Gunakan 'long long' untuk menghindari overflow pada grid besar
    long long valid_cell_count = 0; 
    
    // Loop melalui array valid_cell yang sudah diisi oleh read_dem()
    for (int j = 0; j < NY; j++) {
        for (int i = 0; i < NX; i++) {
            if (valid_cell[j][i] == 1) {
                valid_cell_count++;
    ...
    // Ambil luas per sel dari konstanta di header
    float cell_area_m2 = DX * DY; 
    
    // Gunakan 'double' untuk presisi tinggi pada area total
    double total_valid_area_m2 = (double)valid_cell_count * cell_area_m2;
    double total_bounding_box_area_m2 = (double)(NX * NY) * cell_area_m2;
    double percentage_valid = (total_valid_area_m2 / total_bounding_box_area_m2) * 100.0;

    printf("--- Statistik Area DAS ---\n");
    printf("Total Sel Grid (NX*NY) : %d\n", NX * NY);
    printf("Jumlah Sel Valid (DAS) : %lld\n", valid_cell_count);
    printf("Area per Sel (DX*DY)   : %.4f m^2\n", cell_area_m2);
    printf("Area Bounding Box Total: %.2f m^2 (%.0f Ha)\n", total_bounding_box_area_m2, total_bounding_box_area_m2 / 10000.0);
    printf("==================================================\n");
    printf("Total Area Valid (DAS) : %.2f m^2 (%.0f Ha)\n", total_valid_area_m2, total_valid_area_m2 / 10000.0);
    printf("Persentase Area Valid  : %.2f%%\n", percentage_valid);
    printf("==================================================\n\n");
    // --- AKHIR KODE TAMBAHAN ---
    ...
    if (rain_pattern == 5) {
        ...
        if (abm_hyetograph_num_blocks > 0) {
            ...
            if (abm_hyetograph_m_per_s) {
                printf("Menghitung Hyetograph ABM...\n");
    ...
    GLFWwindow *window = glfwCreateWindow(window_width, window_height, "Simulasi Banjir CUDA (Mass-Conservative)", NULL, NULL);
    ...
    printf("Mengalokasikan memori di GPU (%.2f GB)...\n", (size_float * 7 + size_int + NX * NY * sizeof(curandState)) / (1024.0*1024.0*1024.0));
    CUDA_CHECK(cudaMalloc((void**)&d_z, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_eta, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_eta_new, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_p, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_q, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_n_manning, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_valid_cell, size_int));
    CUDA_CHECK(cudaMalloc((void**)&d_rain_source, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_overlay, size_float));
    CUDA_CHECK(cudaMalloc((void**)&d_rand_states, NX * NY * sizeof(curandState)));

    // Inisialisasi random generator CUDA
    dim3 blockSize(16, 16);
    dim3 gridSize((NX + blockSize.x - 1) / blockSize.x, (NY + blockSize.y - 1) / blockSize.y);
    setup_curand_kernel<<<gridSize, blockSize>>>(d_rand_states, time(NULL));
    CUDA_CHECK(cudaGetLastError());


    printf("Menyalin data awal ke GPU...\n");
    CUDA_CHECK(cudaMemcpy(d_z, z, size_float, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_eta, eta, size_float, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_n_manning, n_manning, size_float, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_valid_cell, valid_cell, size_int, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_overlay, overlay_data, size_float, cudaMemcpyHostToDevice));
    ...
    FILE *logfile = fopen("log_simulasi_mass_conservative.csv", "w");
    ...
    // --- PERBAIKAN: Variabel visualisasi yang hilang ---
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!paused && !simulation_finished) {
            ...
            if (current_sim_time >= TOTAL_SIMULATION_DURATION_SECONDS) {
                ...
                // --- PERBAIKAN: Menggunakan 'current_dt' bukan 'DT' ---
                float rain_depth_this_step_mm = (uniform_rain_intensity_m_per_s * current_dt) * 1000.0f;
                cumulative_rainfall_mm += rain_depth_this_step_mm;
                compute_rainfall_kernel<<<gridSize, blockSize>>>(d_rain_source, d_valid_cell, rain_pattern, current_sim_time, uniform_rain_intensity_m_per_s, d_rand_states);
                
                simulate_step_cuda(current_dt);

                CUDA_CHECK(cudaEventRecord(stop_gpu));
                CUDA_CHECK(cudaEventSynchronize(stop_gpu));
                float ms;
                CUDA_CHECK(cudaEventElapsedTime(&ms, start_gpu, stop_gpu));
                total_parallel_time += ms / 1000.0;
                
                current_sim_time += current_dt;
                total_steps_taken++;

                double serial_part2_start = GET_TIME();
                if (total_steps_taken % LOG_INTERVAL == 0 || total_steps_taken == 1) { // --- PERBAIKAN: Log di langkah pertama ---
                ...
                render_gl(); 
                double serial_part2_end = GET_TIME();
                total_serial_time += (serial_part2_end - serial_part2_start);
            }
        }

        double serial_part3_start = GET_TIME();
        glfwSwapBuffers(window);
        glfwPollEvents();
        double serial_part3_end = GET_TIME();
        if (!paused && !simulation_finished) {
             total_serial_time += (serial_part3_end - serial_part3_start);
        }
        ...
        snprintf(title_buffer, sizeof(title_buffer), "Simulasi Banjir | Status: %s | Time: %.2f s | Max h: %.4f m | S=SS, D=Data, O=Overlay, Spasi=Pause", 
    ...
    CUDA_CHECK(cudaFree(d_eta_new));
    CUDA_CHECK(cudaFree(d_p));
    CUDA_CHECK(cudaFree(d_q));
    CUDA_CHECK(cudaFree(d_n_manning));
    CUDA_CHECK(cudaFree(d_valid_cell));
    CUDA_CHECK(cudaFree(d_rain_source));
    CUDA_CHECK(cudaFree(d_overlay));
    CUDA_CHECK(cudaFree(d_rand_states));

    // Performa GPU dan CPU
    printf("\n--- Laporan Performa Simulasi Rinci ---\n");
    printf("Total Waktu Eksekusi (Wall Clock) : %.4f detik\n", wall_clock_time_used);
    printf("Total Langkah Simulasi            : %d langkah\n\n", total_steps_taken);

    double total_measured_time = total_parallel_time + total_serial_time;
    
    printf("--- Rincian Waktu Komputasi ---\n");
    printf("Waktu Paralel (GPU Kernels)       : %.4f detik (%.2f%%)\n", total_parallel_time, (total_parallel_time / wall_clock_time_used) * 100.0);
    printf("Waktu Serial (CPU Tasks)          : %.4f detik (%.2f%%)\n", total_serial_time, (total_serial_time / wall_clock_time_used) * 100.0);
    
    double unmeasured_time = wall_clock_time_used - total_measured_time;
